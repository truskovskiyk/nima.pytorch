...
from nima.cli import get_image_score


def test_get_image_score(cli_runner: CliRunner, state_dict_path: Path, image_path: Path):
    result = cli_runner.invoke(get_image_score,
                               ['--path_to_model_state', state_dict_path, '--path_to_image', image_path])
...
def test_prepare_dataset(cli_runner: CliRunner):
    result = cli_runner.invoke(prepare_dataset, ['--path_to_ava_txt', 'test.csv',
                                                 '--path_to_save_csv', 'data',
                                                 '--path_to_images', 'path_to_imgsage']
                               )
