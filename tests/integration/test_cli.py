...
class TestCli:
    ...
    def test_prepare_dataset(self, cli_runner: CliRunner, ava_csv_path: Path, path_to_save_result: Path, images_path: Path):
    ...
    def test_train_model(self, cli_runner: CliRunner, images_path: Path, experiment_dir: Path, folder_with_csv: Path):
        ...
        result = cli_runner.invoke(train_model, f"--path_to_save_csv {folder_with_csv} --path_to_images {images_path} --experiment_dir {experiment_dir} --batch_size 1 --num_epoch 5")
    ...
    def test_validate_model(self, cli_runner: CliRunner, images_path: Path, state_dict_path: Path, folder_with_csv: Path):
        result = cli_runner.invoke(validate_model, f"--path_to_model_state {state_dict_path} --path_to_save_csv {folder_with_csv} --path_to_images {images_path}")
        assert result.exit_code == 0
